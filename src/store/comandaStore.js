    ...
    actions: {
        ...
        async fetchComandaById(id_comanda) {
            ...
            if (data && data.comandas_productos && Array.isArray(data.comandas_productos)) {
                ...
                data.comandas_productos = data.comandas_productos.map(({ productos, ...cp }) => ({
                    ...cp,
                    producto: productos,
        ...
        async createComanda(cliente, productosSeleccionados, created_by, id_master) {
            ...
            try {
                const { data: comanda, error: comandaError } = await supabase
                    ...
                    .select();
                console.log('COMANDA CREADA[STORE]: ', comanda);
        ...
        async closeComanda(id_comanda) { //DMO: v.2 Añadir historico
        ...
        async deleteProducto(id_comanda, id_producto) {
            const { error } = await supabase
                .from('comandas_productos')
                .delete()
                .match({ id_comanda: id_comanda, id_producto: id_producto });
            ...
            this.comanda.comandas_productos = this.comanda.comandas_productos.filter(pedido => pedido.producto.id !== id_producto);
